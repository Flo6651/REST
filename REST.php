...
class REST{
   public $verbose=false;   // if true
                           // debugginginformation is printed
   ...
   function __construct($data){
      ...
      switch($this->request[0]){     //selceting the language
         case "XML":
            $this->output = 
               array("Error"=>"unsupported language");
            $this->success=false;
         break;
         case "JSON":
            $this->input = 
               json_decode(@$_SERVER['CONTENT'],true);
         break;
         default:
            $this->output = 
               Array("Error"=>"wrong language");
            $this->success=false;
      break;
      }
   }
    ...
    * 
    * @param $default
    *    contains the return value if no key is found
    * 
    * @param $type
